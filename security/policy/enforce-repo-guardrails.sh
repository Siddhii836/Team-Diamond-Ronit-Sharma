...
require_no_match() {
  local pattern="$1"
  local description="$2"
  if rg -n --glob '!**/node_modules/**' --glob '!**/package-lock.json' "$pattern" "$ROOT_DIR" >/dev/null 2>&1; then
    fail "$description"
  else
    pass "$description"
  fi
...
require_match "from '../config/env.ts'|from '../config/env'" "$ROOT_DIR/securepay/server/src/controllers/authController.ts" "Securepay auth controller uses centralized env config"
require_match "from '../config/env.ts'|from '../config/env'" "$ROOT_DIR/securepay/server/src/middleware/authMiddleware.ts" "Securepay auth middleware uses centralized env config"

# 5) Securepay encryption must use AES-GCM.
require_match "aes-256-gcm" "$ROOT_DIR/securepay/server/src/services/encryptionService.ts" "Securepay encryption uses AES-256-GCM"
