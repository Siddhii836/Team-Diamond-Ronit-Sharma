...
Test-Match -Pattern "from '../config/env.ts'|from '../config/env'" -File (Join-Path $RootDir 'securepay\server\src\controllers\authController.ts') -Description 'Securepay auth controller uses centralized env config'
Test-Match -Pattern "from '../config/env.ts'|from '../config/env'" -File (Join-Path $RootDir 'securepay\server\src\middleware\authMiddleware.ts') -Description 'Securepay auth middleware uses centralized env config'

# 5) Securepay encryption must use AES-GCM.
Test-Match -Pattern 'aes-256-gcm' -File (Join-Path $RootDir 'securepay\server\src\services\encryptionService.ts') -Description 'Securepay encryption uses AES-256-GCM'
